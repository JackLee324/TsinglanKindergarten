...
## 附录：第一阶段部署架构 —— 单实例（硬性约束）

**这是本阶段的正式生产架构前提，不是建议。**

| 项 | 结论 |
|----|------|
| 第一阶段实例数 | **只允许 1 个应用实例**（1 个进程） |
| 限流实现 | **进程内限流**（`server/modules/auth/auth.service.ts` 的 `ipRateMap`，内存 `Map`） |
| 适用范围 | 该限流器**仅在单进程内有效** |
| 多实例直接扩容 | **禁止**。未经改造直接扩展为多实例，会使每个实例各持一套计数器，**有效限流阈值随实例数成倍放宽**，登录暴力破解防护**静默失效**（不会报错、不会告警，只是不再起作用） |
| 扩容前置条件 | **必须先把限流迁移到共享存储**（Redis / shared limiter store），再扩容 |
| 不受影响的部分 | 会话吊销基于数据库 `permissionsVersion`，**多实例下仍然正确**（非内存态） |

### 运维动作要求

1. 部署时**只启动一个应用进程**。
2. **不启用**横向自动扩缩容（HPA / 多副本 / 多容器）。
3. 滚动重启期间若短暂存在两个进程，需知悉**限流在此期间是分裂的**，属于可接受的短暂窗口。
4. 把"再加一个实例"**视为需要先做限流迁移的变更**，而不是容量调整。
5. 监控若发现入站流量来源 IP 的限流命中率异常下降，**首先怀疑是否被部署成了多实例**。

### 未来扩容检查清单（迁移到共享限流存储时）

- [ ] 引入共享存储（Redis 等）并确认其自身的高可用
- [ ] 将 `ipRateMap` 替换为共享实现，保留相同的窗口与阈值语义
- [ ] 确认共享存储不可用时的降级策略是**失败关闭**（拒绝）还是失败开放（放行），并明确记录该选择
- [ ] 回归：多实例下同一 IP 的限流计数必须共享
- [ ] 更新本文档与 `SECURITY.md` 中"限流为进程内实现"的已知缺口条目
