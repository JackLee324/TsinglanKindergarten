...
| **C-4** | 依赖漏洞：**6 HIGH / 0 CRITICAL**（升级前 16 HIGH / 0 CRITICAL，另有 57 moderate；现 total 76→66）。本轮升级 11 个包，其中**唯一真正可达的一项已修复**：**`qs` 6.14.2→6.16.0**——它是 **Express 的查询串解析器、位于请求路径上**，此前三条 moderate DoS 是真实可达的；配套 `body-parser` 1.20.4→1.20.8、`express` 4.22.2→4.22.3（express 自身无公告，升级是为了让 `qs` 约束从 `~6.15.1` 变为 `~6.16.0`）。已用 `createRequire` 实测每个 express/body-parser 实例（含 `@nestjs/platform-express` 的嵌套 express）**全部解析到同一个 `qs@6.16.0`**。其余升级：`drizzle-orm` 0.45.3、`multer` 2.4.0、`lodash` 4.18.1、`js-yaml` 4.3.2、`picomatch` 4.0.7、`@nestjs/cli` 下 `glob` 10.5.0。剩余 6 项 HIGH **全部确认无路可走**：OpenTelemetry ×3 **不可达**（全树 `PrometheusExporter`/`JaegerPropagator` **零引用**，且平台每个已发布版本都锁 `sdk-node@^0.208.0`，0.x 下 caret 不跨 minor，修复版在 9 个 minor 之后）＋ `tmp` 修复需 `>=0.2.6` 而 `external-editor` 全部 16 个版本都锁 `^0.0.33`，且**不在生产树、应用全局零 `child_process`**。门禁**仍拒绝豁免**：`FAIL [20] npm audit reports 6 HIGH`。证据 `evidence/audit-triage/TRIAGE-SUMMARY.md` | 已大幅收敛；残余 6 项均有取证论证 |
