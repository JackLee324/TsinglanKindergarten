...
import { Heart, BookOpen, Languages, Dumbbell, ArrowRight, GraduationCap } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        ...
        const k = structures.find((s: ProgramStructure) => s.program === 'k') ?? null;
        if (mounted) setStructure(k);

        const countMap: Record<string, number> = {};
        await Promise.all(
          K_SUBJECTS.map(async (sub: SubjectCardInfo) => {
            try {
        ...
        logger.error('Failed to load K structure', String(err));
      } finally {
